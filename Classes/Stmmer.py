...
def main(argv):
    ...
    parser.add_option('-e', action="store", dest="expected_stem_location",
                      help="Location of file containing document stemmed by user")
    parser.add_option('-b', action="store", dest="black_list_file_location",
                      help="Location of file containing words not to stem")
    print
    parser.parse_args()
    ...
    "Debug:", debug
    print
    "Blacklist:", black_list_file_location
    print
    "ExpectedFile:", expected_stem_location
    expected = None
    lines_parsed = 0
    ...
    if evaluate:
        ...
        for line in sys.stdin:
            ...
            if len(expected_words) != len(line.split()):
                print
                "Number of words in expected line and stemmed line is different" \
                "- skipping (line " + str(lines_parsed) + ")"
                print
                "Skipped line: ", expected_line
                continue
            stemmed_line = ''
            for word in line.split():
                ...
                if debug and evaluate:
                    print
                    word + '|' + stem + '|' + expected_words[i].lower().decode('utf-8'),
                elif debug:
                    print
                    word + '|' + stem,
                else:
        ...
        ''
        print
        "Good:", stemmed_good, ", bad:", stemmed_bad
...
def get_expected_stem_file(location):
    print
    'The file with the prepared stems is located in: ', location
...
def get_blacklist(location):
    print
    'The file with list of words not to stem was named: ' + location
